...
namespace CC.DialogueSystem
{
    ...
    public abstract class BaseDialogueUIController : MonoBehaviour
    {
        ...
        public void ShowSentence(string speaker, TextModifications textMods, Sprite characterSprite,
        ...
        protected virtual IEnumerator showSentence(string speaker, Sprite characterSprite,
            bool sameSpeakerAsLastDialogue = true, bool autoProceed = false)
        {
            return null;
        }
        ...
        protected IEnumerator processTagsForPosition(int index)
        {
            ...
            foreach (var mod in mods)
                ...
                {
                    changeCharacterSprite(SpriteRepo.Instance.RetrieveSprite(
                        (mod as SimpleModification).GetValue<string>(),
                        (mod as ComplexModification).GetContent<string>()));
                }
                else if (mod.ModType == TextModifications.Modifications.ACTION_WITH_MESSAGE)
                {
                    DialogueController.Instance.PerformActionWithMessage((mod as SimpleModification).GetValue<string>(),
                        (mod as ComplexModification).GetContent<string>());
                }
                else if (mod.ModType == TextModifications.Modifications.ACTION_WITH_TARGET)
                {
                    DialogueController.Instance.PerformActionWithTarget((mod as SimpleModification).GetValue<string>(),
                        (mod as ComplexModification).GetContent<string>());
                }
        }
