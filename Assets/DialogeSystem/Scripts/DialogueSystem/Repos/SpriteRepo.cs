    ...
    public class SpriteRepo
    {
        ...
        public void RegisterCharacterSprites(DialogueSprites_SO sprites)
        {
            if (_characterSprites.ContainsKey(sprites.CharactersName))
                DialogueLogger.LogWarning(
                    $"Character sprites for {sprites.CharactersName} already exist, overwritting");
        ...
        private bool validateSprites(DialogueSprites_SO sprites)
        {
            ...
            if (sprites.CharacterSprites.Count == 0)
                DialogueLogger.LogWarning(
                    $"Trying to register character sprites for {sprites.CharactersName} but the sprite list is empty");
            ...
            foreach (var spritePair in sprites.CharacterSprites)
            {
                ...
                if (string.IsNullOrEmpty(spritePair.Name) || string.IsNullOrWhiteSpace(spritePair.Name))
                {
                    DialogueLogger.LogError(
                        $"Error registering character sprites for {sprites.CharactersName}, one or more of the names is empty");
                    return false;
                }
                ...
                {
                    DialogueLogger.LogError(
                        $"Error registering character sprites for {sprites.CharactersName}, there is already a sprite with the name {spritePair.Name}. Each sprite must have a unique name");
                    return false;
                }
                ...
                {
                    DialogueLogger.LogError(
                        $"Error registering character sprites for {sprites.CharactersName} with the sprite name {spritePair.Name}, but the sprite is empty");
                    return false;
                }
            }
        ...
        public Sprite RetrieveSprite(string character, string name = "Default")
        {
            ...
            {
                DialogueLogger.LogError(
                    $"trying to retrieve sprite {name} for the character {character}, but it is not registered in the repo.");
                return null;
            }
