...
namespace CC.DialogueSystem
{
    ...
    public abstract class BaseBackgroundDialogueUIController : MonoBehaviour
    {
                ...
                if (_currentDialogue.NextId != -1)
                {
                    ...
                    if (nextDialogue == null)
                    {
                        DialogueLogger.LogError(
                            $"Trying to navigate to a dialogue with the Id {_currentDialogue.NextId}, but there's isn't one present in the current conversation");
                        return;
                    }
        ...
        // Show a sentence
        protected virtual IEnumerator showSentence(TextModifications textMod)
        {
            return null;
        }
        ...
        protected IEnumerator processTagsForPosition(TextModifications textMod, int index)
        {
            ...
            foreach (var mod in mods)
            {
                ...
                {
                    BackgroundDialogueController.Instance?.StartConversation((mod as SimpleModification)
                        .GetValue<string>());
                }
                ...
                else if (mod.ModType == TextModifications.Modifications.SEND_MESSAGE)
                {
                    ...
                    {
                        DialogueLogger.LogError(
                            $"Trying to execute a send message command, but GameObject {(mod as SimpleModification).GetValue<string>()} was not found");
                        continue;
                    }

                    revievingObject.SendMessage((mod as ComplexModification).GetContent<string>(),
                        SendMessageOptions.DontRequireReceiver);
                }
                else if (mod.ModType == TextModifications.Modifications.ACTION_WITH_MESSAGE)
                {
                    performActionWithMessage((mod as SimpleModification).GetValue<string>(),
                        (mod as ComplexModification).GetContent<string>());
                }
                else if (mod.ModType == TextModifications.Modifications.ACTION_WITH_TARGET)
                {
                    performActionWithTarget((mod as SimpleModification).GetValue<string>(),
                        (mod as ComplexModification).GetContent<string>());
                }
            }
        }
        ...
        {
            ActionController.PerformAction(_conversation, actionName);
        }

        private void performActionWithMessage(string actionName, string message)
        {
            ActionController.PerformActionWithMessage(_conversation, actionName, message);
        }

        private void performActionWithTarget(string actionName, string target)
        {
            ActionController.PerformActionWithTarget(_conversation, actionName, target);
        }
        ...
        {
            return GameObject.Find(_currentDialogue.AnchorObject)?.transform;
        }
